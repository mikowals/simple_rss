  ...
  removeFeed: function (feedId) {
    ...
    if ( Meteor.isServer ) {
      ...
      if ( feed.subscribers && feed.subscribers[0] !== self.userId ){
        console.error( 'user: ', self.userId, " feedSub: ", feed.subscribers[0]);
        return false;
      }
    }
  ...
  addFeed: function( doc ){
    ...
    if ( Meteor.isServer ) {
      ...
      if ( existing ) {
        ...
        userId && Meteor.users.update( self.userId, {$addToSet:{ feedList: existing._id }}, handleError);
        return true;
      }
    }
    ...
    if ( Meteor.isServer ){
      ...
      if ( rssResult.url !== originalUrl ) {
        ...
        if ( existing ) {
          ...
          Feeds.update( existing._id, {$addToSet: { subscribers: userId }}, handleError);
          userId && Meteor.users.update( userId, {$addToSet:{ feedList: existing._id }}, handleError);
          return true;
        }
      }
      ...
      userId && Meteor.users.update( userId, {$addToSet:{ feedList: rssResult._id }}, handleError );
    }
    return true;
  }
